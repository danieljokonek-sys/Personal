...
class SetupWizard(tk.Tk):
    ...
    def _build_page4(self):
        ...
        examples = [
            ("My Business", "Primary business — invoices, clients, vendors"),
            ("Rental Property", "Rental income, maintenance, tenant comms"),
            ("Side Project", "Freelance or side-hustle related emails"),
            ("Personal Finance", "Banks, insurance, investments"),
        ]
        for i in range(4):
        ...
        headers.pack(fill="x", pady=(0, 4))
        make_label(headers, "Entity Name", size=9, color=MUTED_FG).pack(side="left", padx=(0, 118))
        make_label(headers, "Description (optional)", size=9, color=MUTED_FG).pack(side="left")

        for i in range(4):
            row = make_frame(frame)
            row.pack(fill="x", pady=4)
            name_e = make_entry(row, width=18)
            name_e.config(textvariable=self.data["entities"][i]["name"])
            name_e.pack(side="left", padx=(0, 8))
            desc_e = make_entry(row, width=32)
    ...
    def _write_config(self):
        ...
        for i in range(4):
            ...
            if ename:
                key = f"entity_{i+1}"
                entities[key] = {"name": ename, "description": edesc}
