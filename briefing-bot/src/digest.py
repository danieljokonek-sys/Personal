...
class DigestGenerator:
    ...
    def collect_digest_data(self) -> dict:
        ...
        Each actionable item gets a short reference code so the user can
        reply to the digest email with e.g. "done A3, done D5" to mark
        items complete.  Codes use a single-letter prefix:
            A = action_items, D = deadlines, M = financial_items,
            T = tasks, F = follow_ups, G = agreements
        followed by the database row id.
        """
        summary = db.get_dashboard_summary()

        def _tag(prefix, items):
            for item in items:
                item["ref"] = f"{prefix}{item['id']}"
            return items
        ...
        return {
            ...
            "upcoming_deadlines": _tag("D", db.get_pending_deadlines(days_ahead=14)),
            "money_owed_to_you": _tag("M", db.get_pending_financial(direction="receivable")),
            "money_you_owe": _tag("M", db.get_pending_financial(direction="payable")),
            "active_agreements": _tag("G", db.get_active_agreements()),
            "action_items": _tag("A", db.get_pending_actions()),
            "tasks": _tag("T", db.get_pending_tasks()),
            "follow_ups_needed": _tag("F", db.get_pending_follow_ups()),
            "stale_action_items": _tag("A", db.get_stale_action_items(days=3)),
...
<div class="footer" style="margin-top:32px; padding-top:16px; border-top:1px solid #dee2e6; font-size:0.85em; color:#6c757d;">
  <p><strong>Reply to mark items done:</strong> Just reply to this email with the codes of completed items.<br>
  Examples: <code>done A3</code> &nbsp; <code>done A3 D5 T2</code> &nbsp; <code>done A3, done D5</code></p>
